...
def spawn_actors(world):
    try:
        ...
        ego_spawn = carla.Transform(
            carla.Location(x=189.0, y=8.0, z=0.3),
...
def control_pedestrian_movement(pedestrian, simulation_time, collision_occurred):
...
class RealTimePlotter:
    def __init__(self, max_points=200):
        ...
        try:
            ...
            self.fig.suptitle('Analyse AEB en Temps Réel - Piéton')
    ...
    def _update_plots(self):
        try:
            ...
            self.ax1.axhline(y=3.0, color='r', linestyle='--', label='Seuil collision')
            self.ax1.set_ylabel('Distance (m)')
            self.ax1.set_title('Distance Ego-Piéton')
            self.ax1.legend()
            self.ax1.grid(True)
            ...
            self.ax3.clear()
            self.ax3.plot(list(self.times), list(self.ego_speeds), 'g-', linewidth=2, label='Véhicule')
...
def main():
    try:
        ...
        while True:
            ...
            pedestrian_visible = control_pedestrian_movement(pedestrian, simulation_time, collision_occurred)
            ...
            if pedestrian_visible and distance < 5.0:
            ...
            if ego_car_stop or collision_occurred:
                ...
                control.throttle = max(2.0, 0.4 - deceleration)
                control.brake = min(1.0, deceleration)

            ego_vehicle.apply_control(control)

            if distance <= 3.0 and not collision_occurred:
